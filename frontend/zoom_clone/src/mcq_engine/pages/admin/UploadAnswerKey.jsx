  ...
  return (
    <div style={{ maxWidth: '900px', margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '28px' }}>
        ...
        <div>
          <h1 style={{ fontSize: '24px', fontWeight: '700', color: 'var(--text-primary)', marginBottom: '2px' }}>Upload Answer Key</h1>
          <p style={{ fontSize: '14px', color: 'var(--text-muted)', margin: 0 }}>{testTitle} · {answeredCount}/{questions.length} answered</p>
        </div>
      </div>
      ...
      {questions.length === 0 ? (
        <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--border-color)', borderRadius: '16px', padding: '48px', textAlign: 'center' }}>
          ...
          <button onClick={() => navigate(`/admin/tests/${id}/questions`)} className="dms-btn dms-btn-primary" style={{ marginTop: '16px' }}>
            Add Questions
          </button>
        </div>
      ) : (
        <>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '16px 20px', background: 'var(--bg-hover)', borderRadius: '12px', marginBottom: '20px', border: '1px solid var(--border-color)', flexWrap: 'wrap' }}>
            <span style={{ fontSize: '14px', fontWeight: '600', color: 'var(--text-secondary)' }}>Bulk Fill:</span>
            {['A','B','C','D','E'].map(opt => (
              <button key={opt} onClick={() => setBulkOption(opt)} style={{ width: '36px', height: '36px', borderRadius: '8px', border: '1px solid var(--border-input)', background: bulkOption === opt ? '#0e71eb' : 'var(--bg-surface)', color: bulkOption === opt ? '#fff' : 'var(--text-primary)', fontSize: '14px', fontWeight: '700', cursor: 'pointer' }}>
                {opt}
              </button>
            ))}
            <button onClick={handleBulkFill} disabled={!bulkOption} className="dms-btn dms-btn-sm dms-btn-outline" style={{ opacity: !bulkOption ? 0.5 : 1 }}>
              Apply to All
            </button>
          </div>
          ...
          <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--border-color)', borderRadius: '16px', overflow: 'hidden', marginBottom: '24px' }}>
            {questions.map((q, i) => (
              <div key={q._id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 20px', borderBottom: '1px solid var(--border-color)', gap: '16px', flexWrap: 'wrap' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '16px', flex: 1, minWidth: '240px' }}>
                  <span style={{ fontSize: '14px', fontWeight: '700', color: '#0e71eb', minWidth: '32px' }}>Q{i+1}</span>
                  <span style={{ fontSize: '14px', color: 'var(--text-primary)' }}>{q.questionText}</span>
                </div>
                <div style={{ display: 'flex', gap: '8px' }}>
                  {['A','B','C','D', ...(q.optionE ? ['E'] : [])].map(opt => (
                    <button key={opt} onClick={() => setAnswer(q.questionNo, opt)} style={{ width: '40px', height: '40px', borderRadius: '8px', border: answers[q.questionNo] === opt ? '2px solid #0e71eb' : '1px solid var(--border-input)', background: answers[q.questionNo] === opt ? '#0e71eb' : 'var(--bg-surface)', color: answers[q.questionNo] === opt ? '#ffffff' : 'var(--text-primary)', fontSize: '14px', fontWeight: '700', cursor: 'pointer', transition: 'all 0.15s' }}>
                      {opt}
                    </button>
                  ))}
                </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
            <button onClick={handleSubmit} disabled={submitting} className="dms-btn dms-btn-primary">
